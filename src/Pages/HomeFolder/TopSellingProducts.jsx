...
//                     src={item?.image[0]}
//                     alt={item?.name}
//                     className={`w-full h-[200px] sm:h-[250px] md:h-[300px] object-cover transition-transform duration-300 ${activeCard === index ? "scale-105" : "group-hover:scale-105"
//                       }`}
//                   />
//                 </Link>
...
/* ------------------ PRODUCT CARD ------------------ */
const ProductCard = ({ item, wishlist, handleWishlist }) => {
  return (
    <div className="group bg-white transition-all duration-300">
      ...
      <div className="relative overflow-hidden rounded-t-lg">
        ...
        >
          <Heart
            className={`w-5 h-5 ${
              wishlist.some((w) => w.productId === item.id)
                ? "fill-rose text-rose"
                : "text-white"
            }`}
          />
      ...
      <p className="text-xs text-gray-500 mt-1 px-2">Nallakkar</p>

      <p className="text-sm font-medium text-gray-800 px-2 line-clamp-2">
        {item?.name}
      </p>
...
export default function TopSellingProducts() {
  ...
  useEffect(() => {
    if (userId) dispatch(fetchCartByUserId(userId));
  }, [dispatch, userId]);
  ...
  const filteredProducts =
    activeTab === "Featured"
      ? products?.filter(
          (p) => Number(p.featuredStatus) === 1 && Number(p.status) === 1
        )
      : products?.filter((p) => Number(p.status) === 1);
    ...
    <section className="px-4 md:px-6 py-11 font-montserrat">
      ...
      <div className="flex justify-center gap-3 md:gap-4 mb-8">
        {tabs.map((tab) => (
          <button
            key={tab}
            onClick={() => tab !== "Best Sellers" && setActiveTab(tab)}
      ...
      <div className="hidden md:grid md:grid-cols-4 lg:grid-cols-5 gap-6 px-12 mb-10">
